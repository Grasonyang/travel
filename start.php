...
if ($_SESSION['pd'] == 0) {
  ...
  $at = mysqli_fetch_array(mysqli_query($db, "SELECT * FROM `projectp` WHERE `projectpwd` LIKE '$_SESSION[codeee]'"));
  if ($at) {
    header("Location:start.php?call=$_SESSION[call]&pd=$_SESSION[codeee]&page=0");
  } else {
    echo "<script>
      if(confirm('this code is wrong')){
        location.href=\"start.php?call=$_SESSION[call]&pd=0&page=-1\";
    ...
    <div class="main" id="pro" style="margin-left:40vw;margin-right:40vw;width:20vw;height:auto;text-align:center;">
      <table id="projec" border="1" style="text-align:center">
        <tr>
          ...
          <td style="font-weight:bolder;width:100px;">code</td>
        </tr>
      </table>
    </div>
    <!-- ADD -->
...
<script>
  ...
  function cha(e) {
    ...
    if (e == 2) {
      ...
      if (aa != 0 && aa != 1 && aa != 3 && aa != 4 && aa != 5 && aa != 6 && aa != 7 && aa != 8 && aa != 9) {
        $(".main").hide();
        ...
        for (let i = 0; i < list.length; i++) {
          ...
          $("#projec").append(`
            <tr>
              ...
              <td>${arr['howmay']}</td>
              <td><a href="start.php?call=<?php echo $_SESSION['call'] ?>&pd=9&page=-1&codeee=${arr['projectpwd']}">${arr['projectpwd']}</a></td>
            </tr>
          `);
        }
      },
